    ...
    for idx, seg in enumerate(segs_all):
        ...
        if cfg.onset_method == "bootstrap":
            auto_lat = detect_mep_onset_bootstrap(
                seg, fs,
                pre_ms=cfg.prestim_ms,  # use full analysis window
...
def run_pipeline(input_path,
    ...
    _deriv_base  = (os.path.join(derivatives_root, "derivatives", meta.sub_ses_path())
                    if derivatives_root
    ...
    for file_i, raw_file in enumerate(txt_files):
        ...
        try:
            ...
            if enable_inspector and show_inspector_cb and segments_final and file_i == 0:
                ...
                _insp_segs = segments_inspector if segments_inspector else segments_final
